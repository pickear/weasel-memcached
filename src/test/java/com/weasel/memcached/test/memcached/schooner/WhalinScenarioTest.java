...
import com.weasel.memcached.memcached.schooner.SockOutputStream;
import com.weasel.memcached.memcached.schooner.TransCoder;
...
public class WhalinScenarioTest extends TestCase {
	...
	public void testMaxConn() {
		...
		assertEquals(pool.getInitConn(), SchoonerSockIOPool.getInstance().getSocketPool().get(hosts[0]).getNumIdle());
		for (int i = 0; i < 20; ++i) {
		...
		assertTrue(SchoonerSockIOPool.getInstance().getSocketPool().get(hosts[0]).getNumActive() < 20);
		mc.flushAll();
		pool.shutDown();
	}
	...
	public void testMinConn() {
		...
		assertEquals(2, SchoonerSockIOPool.getInstance().getSocketPool().get(hosts[0]).getMinIdle());
		thread = new Thread() {
			@Override
			public void run() {
				for (int i = 0; i < 200; ++i) {
		...
		assertEquals(2, SchoonerSockIOPool.getInstance().getSocketPool().get(hosts[0]).getNumActive());
		mc.flushAll();
		pool.shutDown();
	}
